...
The ESP32 performs audio processing functions such as noise filtering, gain control, voice enhancement, and audio formatting to improve speech clarity in noisy riding environments. The processed audio is then converted into analog audio output using the ESP32's built-in DAC and transmitted through ESP's built in bluetooth module.

The ESP wirelessly streams the audio via Bluetooth to a paired Bluetooth earbud or earpiece, allowing clear communication between the rider and pillion passenger.
